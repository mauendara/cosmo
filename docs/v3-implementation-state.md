...
| 82 | New `task.machine._git_commit_uncommitted_specs(repo_path, config)`, called at the top of `_do_merging`'s `try` block, right before `merge_task`/`_assert_ready` runs -- scoped `git add -A -- docs/specs` + commit (`"cosmo: commit docs/specs"`), a no-op when `docs/specs` doesn't exist or has nothing new, raising `GitCommandError` (now also caught alongside `MergeCommandError` in `_do_merging`) on a real git failure | Not named in the spec | Post-v0.1.0 | Requested directly: a real merge failed with `git.merge._assert_ready`'s "has uncommitted changes -- refusing to merge" because `cosmo spec add`/`spec queue` write `docs/specs/**` straight into `repo_path` (the same checkout `_assert_ready` inspects) and never commit it -- the third instance of the same "mutates `repo_path`, never commits it" bug class `bootstrap.git_branch.commit_bootstrap_output` (init output) and `_git_commit_archive` (openspec archive output, deviation 68) already exist to close, this time for spec content. Deliberately scoped to `docs/specs` only, per explicit instruction -- not a general "commit whatever repo_path" fixup. New tests in `test_task_machine.py`: `test_git_commit_uncommitted_specs_commits_new_spec_content`/`_is_a_noop_with_nothing_to_commit`/`_is_a_noop_when_docs_specs_is_absent` exercise the helper directly; `test_uncommitted_docs_specs_no_longer_blocks_the_merge` reproduces the real failure end-to-end through `run_task` (uncommitted `docs/specs` content in `repo_path` before a real merge attempt) and asserts the task now reaches `DONE` with the spec content committed separately from the merge commit |
